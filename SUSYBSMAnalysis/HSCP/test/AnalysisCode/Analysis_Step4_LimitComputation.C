...
public:
  ...
  //// MVA code /////////////
    ...
    stAllInfo(string path=""){
        ...
        EffE          = 0;      EffE_SYSTP    = 0;      EffE_SYSTI    = 0;      EffE_SYSTM    = 0;      EffE_SYSTHUp    = 0;   EffE_SYSTHDown = 0;  EffE_SYSTT    = 0;   EffE_SYSTPU = 0; EffE_SYSTComb = 0; //KENJI
        ...
        fscanf(pFile,"Eff_SystComb : %lf +- %lf\n",&Eff_SYSTComb, &EffE_SYSTComb); //KENJI
        ...
        for( int ihist = 0; ihist < 6; ihist++){
            ...
            fscanf(pFile,(mvanames[ihist]+"Cut  :%lf\n").c_str(),&MVACut[ihist]);
        }
        ///////////////////////////
        fclose(pFile);
    }
    ...
    void Save(string path=""){
        ...
        fprintf(pFile,"Eff_SystComb : %lf +- %lf\n",Eff_SYSTComb, EffE_SYSTComb); //KENJI
...
void Analysis_Step4_LimitComputation(string MODE="COMPILE", string InputPattern="", string signal=""){
    ...
    if(MODE.find("COMPUTELIMIT2016")!=string::npos || MODE.find("OPTIMIZE2016")!=string::npos){
        cout << signal << endl;
        if(signal.find("13TeV16")==string::npos){cout << "found 13TeV16" << endl; return;}
    ...
    fprintf(pFile,             "%20s   Eff   --> PScale |  DeDxScale | MVAScale | HIP | PUScale | TotalUncertainty     \n","Model");
    fprintf(talkFile, "\\hline\n%20s &  Eff    & PScale &  DeDxScale & MVAScale & HIP & PUScale & TotalUncertainty \\\\\n","Model");
    Graphs=0;
    ...
    fprintf(pFile,             "%20s   Eff   --> PScale |  DeDxScale | MVAScale | HIP | PUScale | TOFScale | TotalUncertainty     \n","Model");
    fprintf(talkFile, "\\hline\n%20s &  Eff    & PScale &  DeDxScale & MVAScale & HIP & PUScale & TOFScale & TotalUncertainty \\\\\n","Model");
    Graphs=0;
    for(unsigned int k=0; k<modelVector.size(); k++){
    ...
    TkMVAGraphMap["Gluino_f50"   ]->SetLineColor(4);  TkMVAGraphMap["Gluino_f50"   ]->SetMarkerColor(4);   TkMVAGraphMap["Gluino_f50"   ]->SetLineWidth(2);   TkMVAGraphMap["Gluino_f50"   ]->SetLineStyle(1);  TkMVAGraphMap["Gluino_f50"   ]->SetMarkerStyle(23);
    ///////////////////////////////
    ...
    TkMVAGraphMap["Gluino16_f10"   ]->SetLineColor(4);  TkMVAGraphMap["Gluino16_f10"   ]->SetMarkerColor(4);   TkMVAGraphMap["Gluino16_f10"   ]->SetLineWidth(2);   TkMVAGraphMap["Gluino16_f10"   ]->SetLineStyle(1);  TkMVAGraphMap["Gluino16_f10"   ]->SetMarkerStyle(22);
    ...
    MuMVAGraphMap["Stop16"         ]->SetLineColor(2);  MuMVAGraphMap["Stop16"         ]->SetMarkerColor(2);   MuMVAGraphMap["Stop16"         ]->SetLineWidth(2);   MuMVAGraphMap["Stop16"         ]->SetLineStyle(1);  MuMVAGraphMap["Stop16"         ]->SetMarkerStyle(21);
    ...
    MuMVAGraphMap["GMStau16"       ]->SetLineColor(1);  MuMVAGraphMap["GMStau16"       ]->SetMarkerColor(1);   MuMVAGraphMap["GMStau16"       ]->SetLineWidth(2);   MuMVAGraphMap["GMStau16"       ]->SetLineStyle(1);  MuMVAGraphMap["GMStau16"       ]->SetMarkerStyle(20);
    ...
    TkMVAGraphMap["DY16_Q1"        ]->SetLineColor(46); TkMVAGraphMap["DY16_Q1"        ]->SetMarkerColor(46);  TkMVAGraphMap["DY16_Q1"        ]->SetLineWidth(2);   TkMVAGraphMap["DY16_Q1"        ]->SetLineStyle(1);  TkMVAGraphMap["DY16_Q1"      ]->SetMarkerStyle(20);
    //////////////////////////
    ...
    TkMVAGraphMap["DY16_Q2"        ]->SetLineColor(43); TkMVAGraphMap["DY16_Q2"        ]->SetMarkerColor(43);  TkMVAGraphMap["DY16_Q2"        ]->SetLineWidth(2);   TkMVAGraphMap["DY16_Q2"        ]->SetLineStyle(1);  TkMVAGraphMap["DY16_Q2"      ]->SetMarkerStyle(34);
    //////////////////////////
    ...
    MuMVAGraphMap["Gluino16_f10" ]->Draw("LP");
    MuMVAGraphMap["Gluino16_f50" ]->Draw("LP");
    MuMVAGraphMap["Stop16"       ]->Draw("LP");
    MuMVAGraphMap["GMStau16"     ]->Draw("LP");
    MuMVAGraphMap["PPStau16"     ]->Draw("LP");
    MuMVAGraphMap["DY16_Q1"    ]->Draw("LP");
    MuMVAGraphMap["DY16_Q2"    ]->Draw("LP");
    
    DrawPreliminary(LegendFromType(MuMVAPattern).c_str(), SQRTS, IntegratedLuminosityFromE(SQRTS));
    TLegend* LEGMuMVA = !Combine ? new TLegend(0.60,0.82-7*0.043,0.93,0.82) : new TLegend(0.60,0.15,0.93,0.15+7*0.043);
    ...
    LEGMuMVA->AddEntry(MuMVAGraphMap["DY16_Q1"   ], "|Q| = 1e"                ,"LP");
    LEGMuMVA->AddEntry(MuMVAGraphMap["DY16_Q2"   ], "|Q| = 2e"                ,"LP");
    
    TLegend* LEGThMuMVA = new TLegend(0.15,0.92-(1+6)*0.043,0.50,0.92);
    LEGThMuMVA->SetTextFont(43); //give the font size in pixel (instead of fraction)
    LEGThMuMVA->SetTextSize(18); //font size
    if(!Combine) {
        LEGThMuMVA->SetHeader("Theoretical Prediction");
        LEGThMuMVA->SetFillColor(0);
        //LEGThMuMVA->SetFillStyle(0);
        LEGThMuMVA->SetBorderSize(0);
        
        TGraph* GlThLeg = (TGraph*) ThGraphMap["Gluino_f10"]->Clone("GluinoThLeg");
        GlThLeg->SetFillColor(ThErrorMap["Gluino_f10"]->GetFillColor());
        LEGThMuMVA->AddEntry(GlThLeg, "gluino (NLO+NLL)" ,"LF");
        TGraph* StThLeg = (TGraph*) ThGraphMap["Stop"      ]->Clone("StopThLeg");
        StThLeg->SetFillColor(ThErrorMap["Gluino_f10"]->GetFillColor());
        LEGThMuMVA->AddEntry(StThLeg   ,"stop (NLO+NLL)" ,"LF");
        TGraph* PPStauThLeg = (TGraph*) ThGraphMap["PPStau"        ]->Clone("PPStauThLeg");
        PPStauThLeg->SetFillColor(ThErrorMap["Gluino_f10"]->GetFillColor());
        LEGThMuMVA->AddEntry(PPStauThLeg   ,"stau, dir. prod. (NLO)" ,"LF");
        TGraph* StauThLeg = (TGraph*) ThGraphMap["GMStau"        ]->Clone("StauThLeg");
        StauThLeg->SetFillColor(ThErrorMap["Gluino_f10"]->GetFillColor());
        LEGThMuMVA->AddEntry(StauThLeg   ,"stau (NLO)" ,"LF");
        //TGraph* DYQ2o3ThLeg = (TGraph*) ThGraphMap["DY_Q2o3"        ]->Clone("DYQ2o3ThLeg");
        //DYQ2o3ThLeg->SetFillColor(ThErrorMap["DY_Q2o3"]->GetFillColor());
        //LEGThMuMVA->AddEntry(DYQ2o3ThLeg   ,"|Q| = 2e/3 (LO)" ,"LF");
        TGraph* DYQ1ThLeg = (TGraph*) ThGraphMap["DY16_Q1"        ]->Clone("DYQ1ThLeg");
        DYQ1ThLeg->SetFillColor(ThErrorMap["DY16_Q1"]->GetFillColor());
        LEGThMuMVA->AddEntry(DYQ1ThLeg   ,"|Q| = 1e (LO)" ,"LF");
        TGraph* DYQ2ThLeg = (TGraph*) ThGraphMap["DY16_Q2"        ]->Clone("DYQ2ThLeg");
        DYQ2ThLeg->SetFillColor(ThErrorMap["DY16_Q2"]->GetFillColor());
        LEGThMuMVA->AddEntry(DYQ2ThLeg   ,"|Q| = 2e (LO)" ,"LF");
        LEGThMuMVA->Draw();
    }
    ...
    TkMVAGraphMap["GMStau16"     ]->Draw("LP");
    ...
    TLegend* LEGTkMVA = !Combine ? new TLegend(0.60,0.82-7*0.043,0.93,0.82) : new TLegend(0.60,0.15,0.93,0.15+7*0.043);
    ...
    TLegend* LEGThTkMVA = new TLegend(0.15,0.92-(1+6)*0.043,0.50,0.92);
    LEGThTkMVA->SetTextFont(43); //give the font size in pixel (instead of fraction)
    LEGThTkMVA->SetTextSize(18); //font size
    if(!Combine) {
        LEGThTkMVA->SetHeader("Theoretical Prediction");
        LEGThTkMVA->SetFillColor(0);
        //LEGThTkMVA->SetFillStyle(0);
        LEGThTkMVA->SetBorderSize(0);
        TGraph* GlThLeg = (TGraph*) ThGraphMap["Gluino_f10"]->Clone("GluinoThLeg");
        GlThLeg->SetFillColor(ThErrorMap["Gluino_f10"]->GetFillColor());
        LEGThTkMVA->AddEntry(GlThLeg, "gluino (NLO+NLL)" ,"LF");
        TGraph* StThLeg = (TGraph*) ThGraphMap["Stop"      ]->Clone("StopThLeg");
        StThLeg->SetFillColor(ThErrorMap["Gluino_f10"]->GetFillColor());
        LEGThTkMVA->AddEntry(StThLeg   ,"stop (NLO+NLL)" ,"LF");
        TGraph* PPStauThLeg = (TGraph*) ThGraphMap["PPStau"        ]->Clone("PPStauThLeg");
        PPStauThLeg->SetFillColor(ThErrorMap["Gluino_f10"]->GetFillColor());
        LEGThTkMVA->AddEntry(PPStauThLeg   ,"stau; dir. prod. (NLO)" ,"LF");
        TGraph* StauThLeg = (TGraph*) ThGraphMap["GMStau"        ]->Clone("StauThLeg");
        StauThLeg->SetFillColor(ThErrorMap["Gluino_f10"]->GetFillColor());
        ...
        TGraph* DYQ2ThLeg = (TGraph*) ThGraphMap["DY16_Q2"        ]->Clone("DYQ2ThLeg");
        DYQ2ThLeg->SetFillColor(ThErrorMap["DY16_Q2"]->GetFillColor());
        LEGThTkMVA->AddEntry(DYQ2ThLeg   ,"|Q| = 2e (LO)" ,"LF");
...
TGraph* CheckSignalUncertainty(FILE* pFile, FILE* talkFile, string InputPattern, string ModelName, std::vector<stSample>& modelSample){
    ...
    for(unsigned int s=0;s<modelSample.size();s++){
        ...
        if(modelSample[s].ModelName().find("DY_Q")!=string::npos && modelSample[s].ModelName().find("Q1")!=string::npos) SystMB[N]=-0.2;
    ...
    if(N>0) {
        ...
        if(TypeMode!=0 && TypeMode!=6 && TypeMode!=5)SystGraphs->Add(graphSystT,"LP0"); //Kenji
	...
	//MVA code //////////////////////
        ...
        if(TypeMode > 5 ){
            if(TypeMode!=3)LEG->AddEntry(graphSystI,  "dE/dx" ,"LP");
            if(TypeMode!=3)LEG->AddEntry(graphSystM,  "MVA comb" ,"LP");
            if(TypeMode!=3)LEG->AddEntry(graphSystH,  "HIP" ,"LP");
        }
        ////////////////////////////////
        LEG->AddEntry(graphSystPU,  "Pile Up" ,"LP");
        if(TypeMode!=0 && TypeMode!=6 && TypeMode!=5)LEG->AddEntry(graphSystT,  "1/#beta" ,"LP"); //Kenji
...
TGraph* MakePlot(FILE* pFile, FILE* talkFile, string InputPattern, string ModelName, int XSectionType, std::vector<stSample>& modelSamples, double& LInt){
    ...
    if(XSectionType>0 && FileFound && TypeMode < 6){
    ...
    if( XSectionType>0 && FileFound && TypeMode > 5){
        ...
        else if(ModelName.find("Stop")!=std::string::npos){
            number = 1; //cout << number << endl;
        }
        else if(ModelName.find("GMStau")!=std::string::npos){
            number = 2; //cout << number << endl;
        }
        else if(ModelName.find("PPStau")!=std::string::npos){
            number = 3; //cout << number << endl;
        }
        else if(ModelName.find("Q1")!=std::string::npos){
            number = 4; //cout << number << endl;
        }
        else if(ModelName.find("Q2")!=std::string::npos){
            number = 5; //cout << number << endl;
        }
        else{ cout << "Could not find model name" << endl;exit(0);}
        
        
        for(unsigned int i=0;i<signalPoints.size();i++){
            if(Infos[i].XSec_Exp>100.0)continue; //KENJI this might break things. IDK??
            if(Infos[i].MVACut[number] == 0.0){cout << "something went wrong" << endl; cout << ModelName << " " << Infos[i].Mass << " " << number << endl;  continue;}
            //for(unsigned int i=0;i<N;i++){
            if(Infos[i].WP_TOF==-1){fprintf(pFile,"%-20s & %4.0f & %6.0f & %5.3f & / & %1.3f & %6.3f $\\pm$ %6.3f & %2.0f & %4.3f & %6.1E & %6.1E & %6.1E & %3.2f \\\\\n", ModelName.c_str(), Infos[i].Mass,  Infos[i].WP_Pt,Infos[i].WP_I,Infos[i].MVACut[number], Infos[i].NPred, Infos[i].NPredErr, Infos[i].NData, Infos[i].Eff, Infos[i].XSec_Th,Infos[i].XSec_Exp, Infos[i].XSec_Obs, Infos[i].Significance);
            }else{                  fprintf(pFile,"%-20s & %4.0f & %6.0f & %5.3f & %4.3f & %1.3f & %6.3f $\\pm$ %6.3f & %2.0f & %4.3f & %6.1E & %6.1E & %6.1E & %3.2f \\\\\n", ModelName.c_str(), Infos[i].Mass,  Infos[i].WP_Pt,Infos[i].WP_I,Infos[i].WP_TOF,Infos[i].MVACut[number], Infos[i].NPred, Infos[i].NPredErr, Infos[i].NData, Infos[i].Eff, Infos[i].XSec_Th,Infos[i].XSec_Exp, Infos[i].XSec_Obs, Infos[i].Significance);
            }
            bool IsNeutral = (ModelName.find("N",0)<std::string::npos);
            if(Infos[i].WP_TOF==-1 && (ModelName=="GMSB Stau" || (int)Infos[i].Mass%200==0)) {
                fprintf(talkFile,"%-20s & %4.0f & %6.0f & %5.3f & / & %1.3f & %6.3f $\\pm$ %6.3f & %2.0f & %4.3f & %3.2f \\\\\n", ModelName.c_str(), Infos[i].MVACut[number],  Infos[i].WP_Pt,Infos[i].WP_I,Infos[i].MVACut[number], Infos[i].NPred, Infos[i].NPredErr, Infos[i].NData, Infos[i].Eff, Infos[i].Significance);
                fprintf(talkFile, "\\hline\n");
            }
            if(Infos[i].WP_TOF!=-1 && !IsNeutral) {
                fprintf(talkFile,"%-20s & %4.0f & %6.0f & %5.3f & %4.3f & %1.3f & %6.3f $\\pm$ %6.3f & %2.0f & %4.3f %3.2f \\\\\n", ModelName.c_str(), Infos[i].MVACut[number],  Infos[i].WP_Pt,Infos[i].WP_I,Infos[i].WP_TOF,Infos[i].MVACut[number], Infos[i].NPred, Infos[i].NPredErr, Infos[i].NData, Infos[i].Eff, Infos[i].Significance);
                fprintf(talkFile, "\\hline\n");
            }
        }
    }
...
void Optimize(string InputPattern, string Data, string signal, bool shape, bool cutFromFile){
    ...
    TH2D* MVASignP[6];
    TH2D* MVASignI[6];
    TH2D* MVASignComb[6]; //This might not be necessary
    TH2D* MVASignT[6];
    TH2D* MVASignPU[6];
    TH2D* MVASignHUp[6];
    TH2D* MVASignHDown[6];
    if(TypeMode > 5){
        for( int ihist = 0; ihist < 6; ihist++){
            ...
            MVASignComb[ihist] = (TH2D*)GetObjectFromPath(InputFile, (samples[CurrentSampleIndex].Name + "/" + MVAhistname_systComb[ihist]).c_str());
            ...
            MVASignHUp[ihist] = (TH2D*)GetObjectFromPath(InputFile, (samples[CurrentSampleIndex].Name + "/" +MVAhistname_systHUp[ihist]).c_str());
            MVASignHDown[ihist] = (TH2D*)GetObjectFromPath(InputFile, (samples[CurrentSampleIndex].Name + "/" +MVAhistname_systHDown[ihist]).c_str());
        }
    }
    ///////////////////////////////
    ...
    if( TypeMode > 5){
        for( int ihist = 0; ihist < 6; ihist++){
            ...
            MVASignComb[ihist] ->Scale(norm);
            MVASignT[ihist] ->Scale(norm);
            MVASignPU[ihist] ->Scale(normPU);
            MVASignHUp[ihist] ->Scale(norm);
    ...
    if(!shape && (TypeMode<=2||TypeMode>5)){
        ...
        if(TypeMode < 6){
            TH1D* tmpMassPredProj = ((TH2D*)MassPred)->ProjectionY("MassPredProj"  ,OptimCutIndex+1,OptimCutIndex+1);
            tmpMassPredProj -> Print();
            //TH1D* tmpMassPredProj = MassPred->ProjectionY("MassPredProj",OptimCutIndex+1,OptimCutIndex+1);
            string massdistname1 = signal+"_massdist_Type"+type_mode;
            TCanvas *masscanvas = new TCanvas(massdistname1.c_str(),massdistname1.c_str(),800,800);
            masscanvas->cd();
            tmpMassSignProj->SetLineWidth(2);
            tmpMassSignProj->Draw("hist");
            string output1a = "massdist/"+massdistname1+"_CutIndex"+StrOptimCutIndex+".png";
            string output2a = "massdist/"+massdistname1+"_CutIndex"+StrOptimCutIndex+".C";
            masscanvas->SaveAs(output1a.c_str());
            masscanvas->SaveAs(output2a.c_str());
            delete masscanvas;
            
            string massdistname2 = signal+"_Predmassdist_Type"+type_mode;
            TCanvas *masscanvas2 = new TCanvas(massdistname2.c_str(),massdistname2.c_str(),800,800);
            masscanvas2->cd();
            tmpMassPredProj->SetLineWidth(2);
            tmpMassPredProj->Draw("hist");
            string output1aa = "massdist/"+massdistname2+"_CutIndex"+StrOptimCutIndex+".png";
            string output2aa = "massdist/"+massdistname2+"_CutIndex"+StrOptimCutIndex+".C";
            masscanvas2->SaveAs(output1aa.c_str());
            masscanvas2->SaveAs(output2aa.c_str());
            delete masscanvas2;
            
            delete tmpMassPredProj;
        }
        /////////////////////////////////
        ...
        if(TypeMode>5){
            ...
            int ihistbegin;
            if( signal.find("Gluino")!=std::string::npos){ihistbegin = 0;}
            else if( signal.find("Stop")!=std::string::npos){ihistbegin = 1;}
            else if( signal.find("GMStau")!=std::string::npos){ihistbegin = 2;}
            else if( signal.find("PPStau")!=std::string::npos){ihistbegin = 3;}
            else if( signal.find("Q1")!=std::string::npos){ihistbegin = 4;}
            else if( signal.find("Q2")!=std::string::npos){ihistbegin = 5;}
            for( int ihist = ihistbegin; ihist<ihistbegin+1; ihist++){
                cout << MVAData[ihist]->GetName() << endl;
                TH1D *tmpMVADataproj = MVAData[ihist]->ProjectionY("MVADataProj",OptimCutIndex+1,OptimCutIndex+1);
                
                TH1D *tmpMVASignProj = MVASign[ihist]->ProjectionY("MVASignProj",OptimCutIndex+1,OptimCutIndex+1);
                
                TH1D *tmpMVAPredProj = MVAPred[ihist]->ProjectionY("MVAPredProj",OptimCutIndex+1,OptimCutIndex+1);
                tmpMVADataproj->Print();
                tmpMVAPredProj->Print();
                
                string mvatitle1 = signal+"_"+(MVASign[ihist]->GetName())+"_dist_Type"+type_mode+"_CutIndex"+StrOptimCutIndex;
                TCanvas *cMVA = new TCanvas(mvatitle1.c_str(),mvatitle1.c_str(),800,800);
                cMVA->cd();
                tmpMVASignProj->SetLineWidth(2);
                tmpMVASignProj->Draw("hist");
                string output1b = "MVAdists/"+mvatitle1+".png";
                string output2b = "MVAdists/"+mvatitle1+".C";
                tmpMVASignProj->SaveAs(output1b.c_str());
                tmpMVASignProj->SaveAs(output2b.c_str());
                delete cMVA;
                
                string mvatitle2 = signal+"_"+(MVAPred[ihist]->GetName())+"_dist_Type"+type_mode+"_CutIndex"+StrOptimCutIndex;
                TCanvas *cMVA2 = new TCanvas(mvatitle2.c_str(),mvatitle2.c_str(),800,800);
                cMVA2->cd();
                tmpMVAPredProj->SetLineWidth(2);
                tmpMVAPredProj->Draw("hist");
                string output1bb = "MVAdists/"+mvatitle2+".png";
                string output2bb = "MVAdists/"+mvatitle2+".C";
                tmpMVAPredProj->SaveAs(output1bb.c_str());
                tmpMVAPredProj->SaveAs(output2bb.c_str());
                delete cMVA2;
                ...
                //MVAMinRange[ihist] = std::max(0.0, MVA_mean[ihist]-1*MVA_width[ihist]);
                ...
                double SovrB = 1E-10;
                double BerrovrB = 1.0;
                double optimumMVACut = 0.2;
                double D = 0.0;
                //cout << "RescaleError = " << RescaleError << endl;
                for( int icut = 0; icut < 40; icut++){
                    double cut = 0.2+icut*0.02;
                    S = tmpMVASignProj->IntegralAndError(tmpMVASignProj->GetXaxis()->FindBin(cut),tmpMVASignProj->GetXaxis()->FindBin(2.0),Serr);
                    B = tmpMVAPredProj->Integral(tmpMVAPredProj->GetXaxis()->FindBin(cut),tmpMVAPredProj->GetXaxis()->FindBin(2.0));
                    D = tmpMVADataproj->Integral(tmpMVADataproj->GetXaxis()->FindBin(cut),tmpMVADataproj->GetXaxis()->FindBin(2.0));
                    
                    Berr = pow(B*RescaleError,2);
                    
                    //S /= (samples[CurrentSampleIndex].XSec*LInt);
                    
                    double S2 = tmpMVASignProj->GetBinContent(tmpMVASignProj->GetXaxis()->FindBin(cut));
                    double B2 = tmpMVAPredProj->GetBinContent(tmpMVAPredProj->GetXaxis()->FindBin(cut));
                    
                    //cout << "initial rescale Berr^2 = " << Berr << endl;
                    for(int ibin=tmpMVAPredProj->GetXaxis()->FindBin(cut); ibin<=tmpMVAPredProj->GetXaxis()->FindBin(2.0) ;ibin++){
                        //if(tmpMVAPredProj->GetBinContent(ibin) == 0)continue;
                        Berr+=pow(tmpMVAPredProj->GetBinError(ibin),2);
                        //cout << "ibin = " << ibin << " binerr = " << tmpMVAPredProj->GetBinError(ibin) << " Berr = " << Berr << endl;
                    }
                    Berr=sqrt(Berr);
                    cout <<  cut << " S= " << S << " B= " << B << " Berr= " << Berr << " S/sqrt(B)= " << S/sqrt(B) << " S/sqrt(S+B)= " << S/sqrt(S+B) << " Berr/B= " <<Berr/B << " D = " << D << endl;
                    if(Berr < 1E-2)continue;
                    double tmpSovrB = S/sqrt(S+B);
                    //double tmpSovrB = S/sqrt(B);
                    double tmpBerrB = Berr/B;
                    double diff = fabs(SovrB-tmpSovrB)/SovrB;
                    //I could also do when the two histograms cross which would be scanning over the two histograms and checking when the bincontents swich, i,e when is B > S suddenly become S < B, where S and B are for that bin.
                    //if(diff > 0.1 && tmpBerrB < 0.25){
                    
                    if( tmpSovrB > SovrB && tmpBerrB < 0.25 ){ //might want to make it a less than minimal change
                        SovrB = tmpSovrB;
                        BerrovrB = tmpBerrB;
                        optimumMVACut = cut;
                    }
                ...
                cout << "OptimumMVAcut = " << optimumMVACut << " S/sqrt(B)= " << SovrB << " Berr/B= " << BerrovrB << endl;
                //exit(0);
                
                MVAMinRange[ihist] = optimumMVACut;
                ...
                //Thought, make the MVAMinRange[ihist] equal to the upper 2sigma edge for the Pred, not the low 2sigma edge for the signal. This could ensure enough Pred makes it in for a good cross section calculation
    ...
    for(int CutIndex=0;CutIndex<MassData->GetNbinsX();CutIndex++){
        ...
        //MVA code ///////////////
            ...
            if( signal.find("Gluino")!=std::string::npos){
                ...
                if(!runCombineMVA(true, false, true, InputPattern, signal, CutIndex, shape, true, result, MVAData[1], MVAPred[1], MVASign[1], MVASignP[1], MVASignI[1], MVASignComb[1], MVASignT[1], MVASignPU[1], MVASignHUp[1], MVASignHDown[1], result.MVACut[1])){
                    printf("runCombineMVA did not converge\n");continue;
                }
            }
            else if( signal.find("GMStau")!=std::string::npos){
                if(!runCombineMVA(true, false, true, InputPattern, signal, CutIndex, shape, true, result, MVAData[2], MVAPred[2], MVASign[2], MVASignP[2], MVASignI[2], MVASignComb[2], MVASignT[2], MVASignPU[2], MVASignHUp[2], MVASignHDown[2], result.MVACut[2])){
                    printf("runCombineMVA did not converge\n");continue;
                }
            }
            else if( signal.find("PPStau")!=std::string::npos){
                if(!runCombineMVA(true, false, true, InputPattern, signal, CutIndex, shape, true, result, MVAData[3], MVAPred[3], MVASign[3], MVASignP[3], MVASignI[3], MVASignComb[3], MVASignT[3], MVASignPU[3], MVASignHUp[3], MVASignHDown[3], result.MVACut[3])){
                    printf("runCombineMVA did not converge\n");continue;
                }
            }
            else if( signal.find("Q1")!=std::string::npos){
                cout << "DYQ3" << endl;
                if(!runCombineMVA(true, false, true, InputPattern, signal, CutIndex, shape, true, result, MVAData[4], MVAPred[4], MVASign[4], MVASignP[4], MVASignI[4], MVASignComb[4], MVASignT[4], MVASignPU[4],  MVASignHUp[4], MVASignHDown[4], result.MVACut[4])){
                    printf("runCombineMVA did not converge\n");continue;
                }
            }
            else if( signal.find("Q2")!=std::string::npos){
                cout << "DYQ6" << endl;
                if(!runCombineMVA(true, false, true, InputPattern, signal, CutIndex, shape, true, result, MVAData[5], MVAPred[5], MVASign[5], MVASignP[5], MVASignI[5], MVASignComb[5], MVASignT[5], MVASignPU[5],  MVASignHUp[5], MVASignHDown[5], result.MVACut[5])){
                    printf("runCombineMVA did not converge\n");continue;
                }
            }
        }
    ...
    //MVA code/////
        ...
        if( signal.find("Gluino")!=std::string::npos){
            ...
            runCombineMVA(false, true, true, InputPattern, signal, toReturn.Index, shape, false, toReturn, MVAData[0], MVAPred[0], MVASign[0], MVASignP[0], MVASignI[0], MVASignComb[0], MVASignT[0], MVASignPU[0], MVASignHUp[0], MVASignHDown[0], result.MVACut[0]);
        }
        else if( signal.find("Stop")!=std::string::npos){
            cout << "Stop MVA" << endl;
            runCombineMVA(false, true, true, InputPattern, signal, toReturn.Index, shape, false, toReturn, MVAData[1], MVAPred[1], MVASign[1], MVASignP[1], MVASignI[1], MVASignComb[1], MVASignT[1], MVASignPU[1], MVASignHUp[1], MVASignHDown[1], result.MVACut[1]);
        }
        else if( signal.find("GMStau")!=std::string::npos){
            cout << "GMStau MVA" << endl;
            runCombineMVA(false, true, true, InputPattern, signal, toReturn.Index, shape, false, toReturn, MVAData[2], MVAPred[2], MVASign[2], MVASignP[2], MVASignI[2], MVASignComb[2], MVASignT[2], MVASignPU[2], MVASignHUp[2], MVASignHDown[2], result.MVACut[2]);
        }
        else if( signal.find("PPStau")!=std::string::npos){
            cout << "PPStau" << endl;
            runCombineMVA(false, true, true, InputPattern, signal, toReturn.Index, shape, false, toReturn, MVAData[3], MVAPred[3], MVASign[3], MVASignP[3], MVASignI[3], MVASignComb[3], MVASignT[3], MVASignPU[3], MVASignHUp[3], MVASignHDown[3], result.MVACut[3]);
        }
        else if( signal.find("Q1")!=std::string::npos){
            cout << "DYQ1" << endl;
            runCombineMVA(false, true, true, InputPattern, signal, toReturn.Index, shape, false, toReturn, MVAData[4], MVAPred[4], MVASign[4], MVASignP[4], MVASignI[4], MVASignComb[4], MVASignT[4], MVASignPU[4], MVASignHUp[4], MVASignHDown[4], result.MVACut[4]);
        }
        else if( signal.find("Q2")!=std::string::npos){
            cout << "DYQ2" << endl;
            runCombineMVA(false, true, true, InputPattern, signal, toReturn.Index, shape, false, toReturn, MVAData[5], MVAPred[5], MVASign[5], MVASignP[5], MVASignI[5], MVASignComb[5], MVASignT[5], MVASignPU[5], MVASignHUp[5], MVASignHDown[5], result.MVACut[5]);
        }
    }
    ///////////////
...
bool runCombineMVA(bool fastOptimization, bool getXsection, bool getSignificance, string& InputPattern, string& signal, unsigned int CutIndex, bool Shape, bool Temporary, stAllInfo& result, TH1* MVAData, TH1* MVAPred, TH1* MVASign, TH1* MVASignP, TH1* MVASignI, TH1* MVASignComb, TH1* MVASignT, TH1* MVASignPU, TH1* MVASignHUp, TH1* MVASignHDown, double MVAmin){
    ...
    if(MVAData->InheritsFrom("TH2")){
        ...
        MVASignProjHUp    = ((TH2D*)MVASignHUp)->ProjectionY("MVASignProHUp",CutIndex+1,CutIndex+1);
        ...
        NSignHUp    = (MVASignProjHUp->IntegralAndError(MVASignProjHUp->GetXaxis()->FindBin(MVAmin), MVASignProjHUp   ->GetXaxis()->FindBin(MVAmax), NSignHUpErr ))   / signalsMeanHSCPPerEvent;  NSignHUpErr   /= signalsMeanHSCPPerEvent;
    ...
    cout << "NPredErr = " << NPredErr << endl;
    cout << "NSignP = " << NSignP << endl;
    cout << "NSignI = " << NSignI << endl;
    cout << "NSignPU = " << NSignPU << endl;
    cout << "NSignT = " << NSignT << endl;
    cout << "NSignHUp = " << NSignHUp << endl;
    ...
    */
    ...
    double EffHUp      = NSignHUp/ (result.XSec_Th*result.LInt);
    double EffHDown    = NSignHDown/ (result.XSec_Th*result.LInt);
    double EffComb     = NSignComb / (result.XSec_Th*result.LInt);
    ...
    double EffTErr     = NSignTErr  / (result.XSec_Th*result.LInt);
    double EffPUErr    = NSignPUErr / (result.XSec_Th*result.LInt);
    double EffHUpErr   = NSignHUpErr/ (result.XSec_Th*result.LInt);
    double EffHDownErr = NSignHDownErr/ (result.XSec_Th*result.LInt);
    double EffCombErr  = NSignCombErr / (result.XSec_Th*result.LInt);
    ...
    result.Eff_SYSTHUp = EffHUp;
    result.Eff_SYSTHDown = EffHDown;
    result.Eff_SYSTComb = EffComb;
    result.EffE       = EffErr;
    result.EffE_SYSTP = EffPErr;
    result.EffE_SYSTI = EffIErr;
    result.EffE_SYSTT = EffTErr;
    result.EffE_SYSTPU = EffPUErr;
    result.EffE_SYSTHUp = EffHUpErr;
    result.EffE_SYSTHDown = EffHDownErr;
    result.EffE_SYSTComb = EffCombErr;
    result.NData     = NData;
    result.NPred     = NPred;
    result.NPredErr  = NPredErr;
    result.NSign     = NSign;
    ...
    NSign /= (result.XSec_Th*result.LInt); //normalize signal to 1pb --> actually its the eff
    cout << "Normalized to 1pb Nsign = " << NSign << endl;
    double SignalScaleFactor = 1.0;
    for(unsigned int i=0;i<20 && NSign<1e-1; i++){SignalScaleFactor*=10.0;  NSign*=10.0;}
    if(NPred<0.001) {NPred=0.001; NPredErr=NPred;}
    
    cout << "SignalScaleFactor = " << SignalScaleFactor << endl;
    cout << "Scaled NSign = " << NSign << endl;
    ...
    char CutIndexStr[255];sprintf(CutIndexStr, "SQRTS%02.0fCut%03.0f",SQRTS, result.Index);
    if(Shape){
        cout << "entering Shape based" << endl;
        //prepare the histograms and variation
        //scale to 10fb xsection and to observed events instead of observed tracks
        ...
        MVASignProjHUp->Rebin(2);
        MVASignProjHDown->Rebin(2);
        MVASignProjComb->Rebin(2);
        ...
        saveVariationHistoForLimit(MVASignProj, MVASignProjComb, signal, "comb");
    ...
    double UncEffT=(EffT-Eff)/Eff;
    double UncEffH= (EffHUp-Eff)/Eff;
    double UncEffComb = (EffComb-Eff)/Eff;
    if(TypeMode==6){UncEffT = 0.0;} //Turned off for tracker-only
    double UncEffRe  = -0.02;
    double UncEffTr  = -0.13;
    double UncEffMB  = 0.0;
    
    cout << "Eff EffP EffI EffPU EffT EffH" << endl;
    cout << Eff << " " << EffP << " " << EffI << " " << EffPU << " " << EffT << " " << EffHUp << endl;
    ...
    printf("UncEffP, UncEffI, UncEffPU, UncEffT, UncEffH, UncEffRe, UncEffTr, UncEffMB, UncEffComb \n");
    printf("uncertainties = %f %f %f %f %f %f %f %f %f\n", UncEffP, UncEffI, UncEffPU, UncEffT, UncEffH, UncEffRe, UncEffTr, UncEffMB, UncEffComb);
    if(isnan((float)UncEffPU))UncEffPU=0.0;
    
    double SignalUnc = 1 + sqrt(UncEffP*UncEffP + UncEffI*UncEffI + UncEffPU*UncEffPU + UncEffT*UncEffT + UncEffH*UncEffH + UncEffTr*UncEffTr + UncEffRe*UncEffRe + UncEffMB*UncEffMB + UncEffComb*UncEffComb);
    result.TotalUnc = SignalUnc-1;
