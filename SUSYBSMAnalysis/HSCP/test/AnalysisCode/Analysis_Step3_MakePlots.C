...
void Analysis_Step3_MakePlots()
{
    ...
    //List the TypeModes you wish to plot
    vector<int> ResultsToPlot = {7};
    ...
    if( find(ResultsToPlot.begin(),ResultsToPlot.end(),6) != ResultsToPlot.end() ){
        ...
        MassPrediction(InputPattern, CutIndex,      "Mass", true, "13TeV16_Loose");
        MassPrediction(InputPattern, CutIndexTight, "Mass", true, "13TeV16_Tight");
        CutFlow(InputPattern, CutIndex);
        CutFlow(InputPattern, CutIndexTight);
        CutFlowPlot(InputPattern, CutIndex);
        CutFlowPlot(InputPattern, CutIndexTight);
    ...
    if( find(ResultsToPlot.begin(),ResultsToPlot.end(),7) != ResultsToPlot.end() ){
        InputPattern = "Results/Type7/";  CutIndex = 16; CutIndexTight = 304; CutIndex_Flip=12;
        MassPrediction(InputPattern, CutIndex,      "Mass", true, "13TeV16_Loose");
        MassPrediction(InputPattern, CutIndexTight, "Mass", true, "13TeV16_Tight");
    ...
    //GetSystematicOnPrediction(InputPattern, "Data13TeV16");  //FOR IMPOSSIBLE REASON, THIS FUNCTION CRA \
SHES IF IT IS RUN TOGETHER WITH THE OTHER FUNCTIONS
...
void MassPrediction(string InputPattern, unsigned int CutIndex, string HistoSuffix, bool showMC, string DataName){
    ...
    string signal[6] = {"Gluino_13TeV_M1000_f10","Stop_13TeV_M800","GMStau_13TeV_M494","PPStau_13TeV_M494","DY_13TeV_M600_Q1","DY_13TeV_M600_Q2"};
    string signalLeg[6] = {"Gluino (M = 10000 GeV)", "Stop (M = 800 GeV)", "GMSB Stau (M = 494 GeV)", "PP Stau (M = 494 GeV)", "DY |Q| = 1e (M = 600 GeV)", "DY |Q| = 2e (M = 600 GeV)"};
    ...
    if(DataName.find("13TeV16")!=string::npos){
        ...
        if(IsMVA){
            for( int ihist = 0; ihist < 6; ihist++){
                SName = signal[ihist];
                PredMVA[ihist] = ((TH2D*)GetObjectFromPath(InputFile, string("Data13TeV16/Pred_") + MVAhistname[ihist] ))->ProjectionY(("tmpPred_"+MVAhistname[ihist]).c_str()    ,CutIndex+1,CutIndex+1);
                DataMVA[ihist] = ((TH2D*)GetObjectFromPath(InputFile, string("Data13TeV16/") + MVAhistname[ihist] ))->ProjectionY(("tmpData_"+MVAhistname[ihist]).c_str()    ,CutIndex+1,CutIndex+1);
                
                SignalMVA[ihist] = ((TH2D*)GetObjectFromPath(InputFile, string(SName+"/") + MVAhistname[ihist] ))->ProjectionY(("TmpSignal_"+MVAhistname[ihist]).c_str()    ,CutIndex+1,CutIndex+1);
                //PredMVA[ihist] = GetProjectionFromPath(InputFile, string("Data13TeV/Pred_") + MVAhistname[ihist], CutIndex, "tmpPred_"+MVAhistname[ihist]);
                //DataMVA[ihist] = GetProjectionFromPath(InputFile, string("Data13TeV/") + MVAhistname[ihist], CutIndex, "tmpData_"+MVAhistname[ihist]);
                //SignalMVA[ihist] = GetProjectionFromPath(InputFile, string(SName+"/"     ) + MVAhistname[ihist], CutIndex, "TmpSignal_"+MVAhistname[ihist]);
        ...
        if(IsMVA && !is2016){
            ...
            for( int ihist = 0; ihist < 6; ihist++){
                SName = signal[ihist];
                PredMVA[ihist] = ((TH2D*)GetObjectFromPath(InputFile, string("Data13TeV/Pred_") + MVAhistname[ihist] ))->ProjectionY(("tmpPred_"+MVAhistname[ihist]).c_str()    ,CutIndex+1,CutIndex+1);
                DataMVA[ihist] = ((TH2D*)GetObjectFromPath(InputFile, string("Data13TeV/") + MVAhistname[ihist] ))->ProjectionY(("tmpData_"+MVAhistname[ihist]).c_str()    ,CutIndex+1,CutIndex+1);
    ...
    if(Data13TeV16 && Pred13TeV16){
        ...
        if(IsMVA){
            ...
            printf("Obs vs Precition MVA \n");
            
            for( int ihist = 0; ihist < 6; ihist++){
                printf("MVA obs vs Pred for %s \n",MVAhistname[ihist].c_str());
                if(ihist == 0){
                    cout << "Total obs = " << DataMVA[ihist]->Integral() << endl;
                    cout << "Total pred = " << PredMVA[ihist]->Integral() << endl;
                }
                //DataMVA[ihist]->Print();
                //PredMVA[ihist]->Print();
                DataMVA[ihist]->Rebin(4);
                PredMVA[ihist]->Rebin(4);
                for( int cut = 0; cut < 10; cut++){
                    double mvaselection = cut*0.10; //
                    
                    double D = DataMVA[ihist]->Integral( DataMVA[ihist]->GetXaxis()->FindBin(mvaselection), DataMVA[ihist]->GetXaxis()->FindBin(2.0));
                    
                    double P = PredMVA[ihist]->Integral( PredMVA[ihist]->GetXaxis()->FindBin(mvaselection), PredMVA[ihist]->GetXaxis()->FindBin(2.0));
                    
                    double Perr = 0;
                    
                    
                    for( int i = PredMVA[ihist]->GetXaxis()->FindBin(mvaselection); i<PredMVA[ihist]->GetXaxis()->FindBin(2.0);i++){
                        Perr += pow(PredMVA[ihist]->GetBinError(i),2);
                    }
                    Perr = sqrt(Perr);
                    printf("%4.2f<MVA<2.0 --> Obs=%9.3f Pred = %9.3f +- %8.3f(syst+stat) %9.3f (syst) %9.3f (stat)\n", mvaselection, D, P, sqrt(Perr*Perr + pow(P*SystError,2)), P*SystError, Perr);
                } //end of cut loop
            }//end of hist loop
    ...
    if(IsMVA){
        for(int ihist = 0; ihist < 6; ihist++){
            MAX[ihist] = 1.0;
            MIN[ihist] = 0.005;
            if(DataMVA[ihist]){//DataMVA[ihist]->Rebin(4);
                Max=std::max(Max, DataMVA[ihist]->GetMaximum());}
            if(PredMVA[ihist]){//PredMVA[ihist]->Rebin(4);
                Max=std::max(Max, PredMVA[ihist]->GetMaximum());}
            if(SignalMVA[ihist]){SignalMVA[ihist]->Rebin(4);
                Max=std::max(Max, SignalMVA[ihist]->GetMaximum());}
            MAX[ihist]*=10.0;
    ...
    if(IsMVA){
        for( int ihist = 0; ihist < 6; ihist++){
            if(PredMVA[ihist])PredMVAErr[ihist] = (TH1D*) PredMVA[ihist]->Clone(("Pred"+MVAhistname[ihist]+"Err").c_str());
            if(PredMVA[ihist]){
                for(unsigned int i=0;i<(unsigned int)PredMVA[ihist]->GetNbinsX();i++){
                    
                    double error = sqrt(pow(PredMVAErr[ihist]->GetBinError(i),2) + pow(PredMVAErr[ihist]->GetBinContent(i)*SystError,2));
                    PredMVAErr[ihist]->SetBinError(i,error);
                    if(PredMVAErr[ihist]->GetBinContent(i)<Min && i>5){
                        for(unsigned int j=i+1;j<(unsigned int)PredMVAErr[ihist]->GetNbinsX();j++)PredMVAErr[ihist]->SetBinContent(j,0);
                    }
                }
            }
        }
...
void PredictionAndControlPlot(string InputPattern, string Data, unsigned int CutIndex, unsigned int CutIndex_Flip){
    ...
    c1->SetLogy(true);
    DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
    if(TypeMode>=2 && TypeMode !=6 )SaveCanvas(c1,InputPattern,string("Control_")+Data+"_Pt_TOFSpectrum");
    c1->SetLogy(false);
    if(TypeMode>=2 && TypeMode !=6 )SaveCanvas(c1,InputPattern,string("Control_")+Data+"_Pt_TOFSpectrumNoLog");
    delete c1;
    ...
    DrawLegend(Histos,legend, "","P", 0.93, 0.88, 0.40, 0.05);
    DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
    if(TypeMode>=2 && TypeMode!=6)SaveCanvas(c1,InputPattern,string("Control_")+Data+"_Is_TOFSpectrumLog");
    
    c1->SetLogy(false);
    //   DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
    //   DrawLegend(Histos,legend, "","P");
    if(TypeMode>=2 && TypeMode!=6)SaveCanvas(c1,InputPattern,string("Control_")+Data+"_Is_TOFSpectrum");
    delete c1;
    ...
    c1->SetLogy(false);
    DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
    if(TypeMode>=2 && TypeMode!=6)SaveCanvas(c1,InputPattern,string("Control_")+Data+"_Im_TOFSpectrum");
    delete c1;
    ...
    for(int i=0; i<PredBins; i++) {
        ...
        c1->SetLogy(true);
        DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
        if(TypeMode>=2 && TypeMode!=6)SaveCanvas(c1,InputPattern,string("Control_")+Data+"_Pt_TOFSpectrum_Binned"+Bin);
        c1->SetLogy(false);
        if(TypeMode>=2 && TypeMode!=6)SaveCanvas(c1,InputPattern,string("Control_")+Data+"_Pt_TOFSpectrumNoLog_Binned"+Bin);
        delete c1;
    }
    ...
    if(TypeMode<3||TypeMode>5) {//These plots only made for analyses using mass distribution
        ...
        DrawLegend(Histos,legend,"", "P", 0.93, 0.88);
        DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
        if(TypeMode>=2 & TypeMode!=6)SaveCanvas(c1,InputPattern,string("Prediction_")+Data+"_TOFSpectrum");
        delete Histos[0]; delete Histos[1];
        delete c1;
        
        if(CutIndex_Flip>0 && TypeMode>=2 && TypeMode != 6){
            ...
            DrawLegend(Histos,legend, "" ,"P", 0.93, 0.88);
            DrawPreliminary(LegendTitle, SQRTS, IntegratedLuminosityFromE(SQRTS));
            if(TypeMode>=2 && TypeMode!=6)SaveCanvas(c1,InputPattern,string("Prediction_")+Data+"_TOFSpectrum_Flip");
            delete Histos[0]; delete Histos[1];
            delete c1;
        }
    }
...
void CutFlowPlot(string InputPattern, unsigned int CutIndex, double ylow, double yhigh, bool setLog){
    ...
    else if (TypeMode==0 || TypeMode==6)NumberOfCuts-=1; //TkOnly does not have TOF cut
...
void GetSystematicOnPrediction(string InputPattern, string DataName){
    ...
    TypeMode = TypeFromPattern(InputPattern);
    if(TypeMode!=2 || TypeMode!=7)return;
    string LegendTitle = LegendFromType(InputPattern);;
...
void CheckPrediction(string InputPattern, string HistoSuffix, string DataType){
    TypeMode = TypeFromPattern(InputPattern);
    if(TypeMode==0 || TypeMode == 6)return;
