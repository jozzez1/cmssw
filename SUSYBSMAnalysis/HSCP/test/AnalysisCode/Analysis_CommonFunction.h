 ...
 public:
  ...
  void setPeriodHIPRate(bool is2016_, const char* ratePdfPixelName=NULL, const char* ratePdfStripName=NULL){
    if (is2016 == is2016_ && ratePdfPixel && ratePdfStrip) return;
      ...
      if (ratePdfStrip) {ratePdfStrip->SetBins016?20:26,0,is2016?10:13); ratePdfStrip->SetNameTitle(tmp2, tmp2);} else ratePdfStrip = new TH1D(tmp2, tmp2,is2016_?20:26,0,is2016_?10:13);
    if (!is2016_){
      ...
      rateP      ratePdfPixel->Scale(1.0/ratePdfPixel->Integral());
      ...
      ratePdtePdfPixel->SetBinContent(2,55300);
      ...
      ratePatePdfPixel->SetBinContent(20,595200);
      ...
      ratePdfStrip->SetBinContent(12,6.454876e+08);       ratePdfStrip->SetBinContent(15,1.033392e+09);
...
void fakeHIP(HitDeDxCollection& hitDeDx){
  for(unsigned int h=0;h<hitDeDx.size();h++){
    hitDeDx[ize_t MaxStripNOM=999, bool correctFEDSat=false, int crossTalkInvAlgo=0, double dropLowerDeDxValue=0.0, dedxHIPEmulator* hipEmulator=NULL);
