...
void Analysis_Step2_BackgroundPrediction(std::string InputPattern="COMPILE")
{
  ...
  cout << "Done Getting Readers" << endl;
  ...
  for(unsigned int S=0; S<Smax; S++) {
	...
	H_H_Binned[i]            = (TH1D*)GetObjectFromPath(directory, ("H_H_Binned" + Version).c_str());
      ...
      //////MVAcode ////////////
      TH3D* Pred_EtaI = (TH3D*)GetObjectFromPath(directory, ("Pred_EtaI" + Suffix).c_str());
      TH3D* Pred_EtaTOF = (TH3D*)GetObjectFromPath(directory, ("Pred_EtaTOF" + Suffix).c_str());
      TH2D* EtaDist = (TH2D*)GetObjectFromPath(directory,("AS_Eta_RegionD" + Suffix).c_str());
	...
	TH2D* Pred_EtaIWeighted = (TH2D*)Pred_EtaI->Project3D("zy");
	TH2D* Pred_EtaIWeighted_PE = (TH2D*)Pred_EtaIWeighted->Clone("Pred_EtaIWeightedPE"); Pred_EtaIWeighted_PE->Reset();
                
	TH2D* Pred_EtaTOFWeighted = (TH2D*)Pred_EtaTOF->Project3D("zy");
	TH2D* Pred_EtaTOFWeighted_PE = (TH2D*)Pred_EtaTOFWeighted->Clone("Pred_EtaTOFWeightedPE"); Pred_EtaTOFWeighted_PE->Reset();
	...
	int TreeStep = NPseudoExp/20;if(TreeStep==0)TreeStep=1;
	...
	for(unsigned int pe=0;pe<NPseudoExp;pe++){
	  ...
	  TH1D* Pred_EtaI_Proj_PE = Pred_EtaIWeighted_PE->ProjectionX("Pred_Eta_ProjPE");
	  ...
	  TH1D *tmpPeta = Pred_EtaPWeighted_PE->ProjectionX("tmpPeta");
	  tmpPeta->Scale(1.0/tmpPeta->Integral());
	  ...
	  TH1D *tmpTOFeta = Pred_EtaTOFWeighted_PE->ProjectionX("tmpTOFeta");
	  tmpTOFeta->Scale(1.0/tmpTOFeta->Integral());
                    
	  double WeightTOF = 0.0;
	  for(int x=0;x<=Pred_EtaTOFWeighted_PE->GetXaxis()->GetNbins()+1;x++){
	    WeightTOF = 0.0;
	    ...
	    for(int y=0;y<=Pred_EtaPWeighted_PE->GetYaxis()->GetNbins()+1;y++){
	      ...
	      Pred_EtaTOFWeighted_PE->SetBinContent(x,y,Pred_EtaTOFWeighted_PE->GetBinContent(x,y)*WeightTOF);
	    }
	  }
	  ...
	  if(TypeMode < 6){
	    ...
	    Pred_P_ProjPE = Pred_EtaPWeighted_PE->ProjectionY("Pred_P_ProjPE");
	    Pred_P_ProjPE->Scale(1.0/Pred_P_ProjPE->Integral());
	  ...
	  if( TypeMode > 5){
	    ...
	    //cout << "Entering MVA loop" << endl;
	    for( int nMVA = 0; nMVA < 6; nMVA++){
	      int combo = 0;
	      double ProbaEta =0.0;
		...
		for(int x=0;x<Pred_P_ProjPEmva->GetNbinsX()+1;x++){    if(Pred_P_ProjPEmva->GetBinContent(x)<=0.0){continue;}  const double& p = Pred_P_ProjPEmva->GetBinCenter(x);
		  ...
		  ProbaP+=Pred_P_ProjPEmva->GetBinContent(x);
                                    
		  for(int y=0;y<Pred_I_ProjPEmva->GetNbinsX()+1;y++){    if(Pred_I_ProjPEmva->GetBinContent(y)<=0.0){continue;}  const double& i = Pred_I_ProjPEmva->GetBinCenter(y);
                                        
		    Proba = 0.0;
                                        
                                        
		    Proba = Pred_P_ProjPEmva->GetBinContent(x) * Pred_I_ProjPEmva->GetBinContent(y) * Pred_Eta_ProjPE->GetBinContent(z);
		    if(Proba<=0 || isnan((float)Proba)){continue;}
			...
			mvavalues[nMVA] = mvaGluinoType6.GetMvaValue(mvainput);
		      }
		      else if( nMVA == 1 && ReaderExists[nMVA]){
			mvavalues[nMVA] = mvaStopType6.GetMvaValue(mvainput);
		      }
		      else if( nMVA == 2 && ReaderExists[nMVA]){
			mvavalues[nMVA] = mvaGMSBStauType6.GetMvaValue(mvainput);
		      }
		      else if( nMVA == 3 && ReaderExists[nMVA]){
			mvavalues[nMVA] = mvaPPStauType6.GetMvaValue(mvainput);
		      }
		      else if( nMVA == 4 && ReaderExists[nMVA]){
			mvavalues[nMVA] = mvaDYQ3Type6.GetMvaValue(mvainput);
		      }
		      else if( nMVA == 5 && ReaderExists[nMVA]){
			mvavalues[nMVA] = mvaDYQ6Type6.GetMvaValue(mvainput);
		      }
			...
			Proba = Pred_P_ProjPEmva->GetBinContent(x) * Pred_I_ProjPEmva->GetBinContent(y) * Pred_Eta_ProjPE->GetBinContent(z) * Pred_T_ProjPEmva->GetBinContent(w);
                                                
			if(Proba<=0 || isnan((float)Proba)){continue;}
                                                
			combo++;
                                                
			vector<double> mvainput(4);
			mvainput[0] = p;
			mvainput[1] = i;
			mvainput[2] = eta;
			mvainput[3] = tof;
			...
			if( nMVA == 0 && ReaderExists[nMVA]){
			  mvavalues[nMVA] = mvaGluinoType7.GetMvaValue(mvainput);
			}
			else if( nMVA == 1 && ReaderExists[nMVA]){
			  mvavalues[nMVA] = mvaStopType7.GetMvaValue(mvainput);
			}
			else if( nMVA == 2 && ReaderExists[nMVA]){
			  mvavalues[nMVA] = mvaGMSBStauType7.GetMvaValue(mvainput);
			}
			else if( nMVA == 3 && ReaderExists[nMVA]){
			  mvavalues[nMVA] = mvaPPStauType7.GetMvaValue(mvainput);
			}
			else if( nMVA == 4 && ReaderExists[nMVA]){
			  mvavalues[nMVA] = mvaDYQ3Type7.GetMvaValue(mvainput);
			}
			else if( nMVA == 5 && ReaderExists[nMVA]){
			  mvavalues[nMVA] = mvaDYQ6Type7.GetMvaValue(mvainput);
			}
		...
		//cout << "Total P Proba = " << ProbaP << endl;
		//if(ProbaP < 1)exit(0);
	...
	for(int x=0;x<Pred_Mass->GetNbinsY()+1;x++){
	  ...
	  ErrTOF=sqrt(ErrTOF/(NPseudoExp-1));
	  ErrComb=sqrt(ErrComb/(NPseudoExp-1));
                    
	  Pred_Mass    ->SetBinContent(CutIndex+1,x,Mean    );
	  Pred_Mass      ->SetBinError(CutIndex+1,x,Err    );
	  Pred_MassTOF ->SetBinContent(CutIndex+1,x,MeanTOF );
	  Pred_MassTOF   ->SetBinError(CutIndex+1,x,ErrTOF );
	  Pred_MassComb->SetBinContent(CutIndex+1,x,MeanComb);
