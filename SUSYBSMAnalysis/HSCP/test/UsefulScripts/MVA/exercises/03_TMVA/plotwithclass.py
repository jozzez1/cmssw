...
def readAndFillAgain(filename, treename, reader, which, c, h):
    ntuple = Ntuple(filename, treename)
    inputvars = vector('double')(3)
    
    # loop over ntuple
    for rownumber, event in enumerate(ntuple):
        inputvars[0] = event.P
        inputvars[1] = event.Ih
        inputvars[2] = event.eta
    ...
    #h.Scale(1.0/h.Integral())
#------------------------------------------------------------------
def AverageDdist(filename, treename, reader, which, h, hcount):
    print "Filling Average D distribution for %s" %filename
    ...
    for rownumber, event in enumerate(ntuple):
        ...
        D = reader.GetMvaValue(inputvars)
        h.Fill(event.P, event.Ih, D+0.5)
        hcount.Fill(event.P, event.Ih,1.0)
...
#    fieldy = 'I';  vary = 'I'
    ...
    #signame2 = "Data13TeV_Run2015DregionTOY_Test_2_Gluino_13TeV_M1000_f10_NoCuts"
    signame2 = "Histos_Data13TeV_Run2015"
    #bkgname = "Data13TeV_Run2015TOY_3982evts_Dregion"
    bkgname = "Data13TeV_Run2015DregionTOY_TEST_1" #bkg file name to test/plot (not training file)
    ...
    bkgvbkgswitch = True #do you want to 3rd file to be REAL DATA?
    etaswitch = True #do you want to compare eta as well?
    ...
    mergedfilename = '../../../Data/Merged_SIG_TOY/%s.root' %signame2
...
#    s2.write('ggF #rightarrow H #rightarrow ZZ #rightarrow 4l')
    ...
    zstep = (zmax-zmin)/zbins

    inputvars = vector('double')(3)
    for k in xrange(zbins):
        z = zmin + (k+0.5)*zstep
        inputvars[2] = z
    ...
    ## ---------------------------------------------------------
    # make 2-D surface plot of Average Discriminant 
    # --------------------------------------------------------- 
    h_sig = mkhist2('h_sig', varx, vary,
                   xbins*10, xmin, xmax,
                   ybins*10, ymin, ymax)
    h_sig_count = mkhist2('h_sig_count', varx, vary,
                   xbins*10, xmin, xmax,
                   ybins*10, ymin, ymax)
    #hsig.SetMarkerSize(msize)
    #hsig.SetMarkerColor(kCyan+1)
    AverageDdist(sigfilename, sigtreename, reader, which, h_sig, h_sig_count)


    # Fill background histogram                                                                                                               
    h_bkg = mkhist2('h_bkg', varx, vary,
                   xbins*10, xmin, xmax,
                   ybins*10, ymin, ymax)
    h_bkg_count = mkhist2('h_bkg_count', varx, vary,
                   xbins*10, xmin, xmax,
                   ybins*10, ymin, ymax)
    #hbkg.SetMarkerSize(msize)
    #hbkg.SetMarkerColor(kMagenta+1)
    AverageDdist(bkgfilename, bkgtreename, reader, which, h_bkg, h_bkg_count)

    # Fill Mixed(merged) file histogram                                                                                                       
    h_merged = mkhist2('h_merged', varx, vary,
                      xbins*10, xmin, xmax,
                      ybins*10, ymin, ymax)
    h_merged_count = mkhist2('h_merged_count', varx, vary,
                      xbins*10, xmin, xmax,
                      ybins*10, ymin, ymax)
    #hmerged.SetMarkerSize(msize)
    #hmerged.SetMarkerColor(kBlack)
    AverageDdist(mergedfilename, mergedtreename, reader, which, h_merged, h_merged_count)
    
    c0_sig = TCanvas("fig_%s_%s_average" %(signame,which), "", 10, 10, 500, 500)
    #cells = h_sig.GetSize()
    #for icell i range(1,cells):
    #    h_sig.SetBinContent(icell,)
    h_sig.Divide(h_sig_count)
    h_sig.Draw("colz")
    c0_sig.SaveAs(".png")

    c0_bkg = TCanvas("fig_%s_%s_average" %(bkgname,which), "", 10, 10, 500, 500)
    h_bkg.Divide(h_bkg_count)
    h_bkg.Draw("colz")
    c0_bkg.SaveAs(".png")
    ...
    hs = mkhist1("hs", "D(%s, %s, %s)" % (varx, vary, varz), "Entries", ndivisions, xm, xmax)
    ...
    hb = mkhist1("hb", "D(%s, %s, %s)" % (varx, vary, varz), "Entries", ndivisions, xm, xmax)
    ...
    hm = mkhist1("hm", "D(%s, %s, %s)" % (varx, vary, varz), "Entries", ndivisions, xm, xmax)
    ...
    if(combinedswitch == True):
        ...
        hdiff.GetYaxis().SetTitle("Mix - Bkg")
        hdiff.Add(hb,-1)
        hdiff.Sumw2()
        ...
        hratio2.GetYaxis().SetTitle("Mix/Bkg")
        hratio2.Sumw2()
        hratio2.Divide(hb)
